...
2. На Starship 7 вам необходимо выполнить полный сброс устройства.
   ...
   6. Если вдруг закрылось инженерное меню, открыть его можно через приложение телефон, введя следующую комбинацию: #*MMDDHH, где MM - текущий месяц +5, DD - текущий день (по китайскому часовому поясу), HH - текущий час (по китайскому часовому поясу в 12часовом формате). Например, если сейчас 2 сентября 15:23 (MSK), то код будет `#*14028`
...
3. Подключите ноутбук к машиной (разъем usbA, обычно справа)
4. Запустите `install_assistant.bat`
